...
function update()
{
	...
	for(i = 0;i < nr;i++)
	{
		r = i<hr?i:nr-i;
		r = Math.pow(r,.5);
		ctx.moveTo(loc[n -hr + i].p.x + r, loc[n -hr + i].p.y);
		ctx.arc(loc[n -hr + i].p.x, loc[n -hr + i].p.y, r, 0, Math.PI*2);
		//ctx.fillRect(loc[n -hr + i].p.x - r, loc[n -hr + i].p.y - r, r*2, r*2, Math.PI*2);
	}
